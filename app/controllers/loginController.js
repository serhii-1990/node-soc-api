  ...
  User.find({ username: loginData.username }, function(err, result) {
    ...
    if (result !== null) {
      ...
      tokens.save(function(err) {
        ...
        else console.log("Tokens was saved!");
      });
    } else {
      res.json(err);
...
exports.refresh = function(req, res) {
  Token.findOne({ refreshToken: req.params.refreshToken }, function(
    err,
    result
  ) {
    ...
    if (result.username !== null && result.isRevoked === false) {
      ...
      const token = jwt.sign(getRefreshToken, config.secret, {
        expiresIn: config.tokenLife
      });
      ...
      tokens.save(function(err) {
        if (err) console.log(err);
        else console.log("Token was updated!");
      });
    } else {
...
exports.revoke = function(req, res) {
  Token.findOne({ refreshToken: req.params.refreshToken }, function(
    ...
    if (err) res.send(err);
      ...
      tokens.save(function(err) {
        if (err) console.log(err);
        else console.log("Token was revoked!");
      });
    }
  });
};
